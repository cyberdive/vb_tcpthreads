...
Public Class Server
        ...
        Public Class SessionCommunications
            ...
            Public writeQueue As New ConcurrentQueue(Of Byte())
            Public initiatingSendFile As Boolean
            Public errorMsg As String

            Public Delegate Sub ReceivedFileProgress(ByVal percentComplete As UInt16)
            Public Delegate Sub SentFileProgress(ByVal percentComplete As UInt16)
            Public Delegate Sub ReceivedFileComplete()
            Public Delegate Sub SentFileComplete()
    
            Public incomingFileProgress As ReceivedFileProgress
            Public outgoingFileProgress As SentFileProgress
            Public incomingFileComplete As ReceivedFileComplete
            Public outgoingFileComplete As SentFileComplete
            Public GetFileCompletionLock As Object = New Object()
            
            Public Function GetPercentOfFileReceived() As UInt16
                Try
                    If ReceivingFile Then
                        Dim ret As UInt16 = CUShort((fileBytesRecieved / IncomingFileSize) * 100)
                        If ret < 101 Then Return ret
                        Return 0
                    Else
                        Return 0
                    End If
            ...
            Public Function GetPercentOfFileSent() As UInt16
                    Try
                    If SendingFile Then
                        Dim ret As UInt16 = CUShort((filebytesSent / outgoingFileSize) * 100)
                        If ret < 101 Then Return ret
                        Return 0
                    Else
                        Return 0
        ...
        Public Sub New(ByVal callbackMethod As ServerCallbackDelegate, Optional ByVal _throttledBytesPerSecond As UInt32 = 9000000, Optional ByVal enforceUniqueMachineId As Boolean = True)
            ...
            blockSize = 62500
            If Mbps < blockSize Then blockSize = CUShort(Mbps)
        ...
        Public Function Start(ByVal prt As Integer, Optional ByRef errorMessage As String = "") As Boolean
            Return Start(prt, GetLocalIpAddress(), errorMessage)
        End Function

        Public Function Start(ByVal prt As Integer, localAddress As IPAddress, Optional ByRef errorMessage As String = "") As Boolean
        ...
        'Public Shared Function GetLocalIpAddress() As System.Net.IPAddress
        ...
        ''' <summary>
        ''' Get a list of V4 IP Addresses
        ''' </summary>
        ''' <returns>A List(Of System.Net.IPAddress)</returns>
        ''' <remarks></remarks>
        Public Shared Function GetLocalIpAddresses() As List(Of System.Net.IPAddress)
            Dim strHostName As String
            Dim alladdresses() As System.Net.IPAddress
            Dim addresses As New List(Of System.Net.IPAddress)

            strHostName     = System.Net.Dns.GetHostName()
            alladdresses    = System.Net.Dns.GetHostAddresses(strHostName)

            ' Find the IpV4 addresses
            For Each address As System.Net.IPAddress In alladdresses
                ' Return the V4 IP Addresses we find in the list.
                If address.AddressFamily = AddressFamily.InterNetwork Then
                    addresses.Add(address)
                End If
            Next

            ' No IP addresses in this system? Add the loopback address:
            If addresses.Count = 0 Then addresses.Add(System.Net.IPAddress.Loopback)
        ...
        Public Function GetSendQueueSize(ByVal sessionId As Int32) As Int32
            ...
            If SessionCollection.GetSession(sessionId, session) Then
                If session IsNot Nothing AndAlso session.IsRunning Then
                    sendQueueSize = session.sendQueue.Count
                End If
            End If
        ...
        Public Function SendFile(ByVal _path As String, ByVal sessionID As Int32, Optional ByVal blockUntilSending As Boolean = True) As Boolean
            ...
            If SessionCollection.GetSession(sessionID, thisSession) then
                ...
                While thisSession.SendingFile
                    Thread.Sleep(1)
                End While

                Dim timeout As Date = Now.AddSeconds(5)
                thisSession.initiatingSendFile = True
                ...
                If blockUntilSending Then
                    While (thisSession.initiatingSendFile AndAlso isRunning AndAlso Now < timeout)
                        Thread.Sleep(1)
                        If thisSession.SendingFile then 
                            thisSession.initiatingSendFile = False
                            Return True
                        End If
                    End While

                    If Now > timeout then errMsg = "Timed out while waiting for file transfer to begin."
                    thisSession.errorMsg = errMsg
        ...
        Public Function SendBytes(ByRef streamBytes As MemoryStream, Optional ByVal channel As Byte = 1, Optional ByVal sessionID As Int32 = -1, _
            ...
            Dim msg As message = New message()
            ReDim msg.bytes(CType(streamBytes.Length, Int32) -1)
            msg.dataChannel = channel
            msg.sessionID = sessionID
        ...
        Private Function BeginFileSend(ByVal _path As String, ByVal session As SessionCommunications, ByVal fileLength As Long) As Boolean
            ...
            Try
                ...
                session.fileReader = New FileStream(_path, FileMode.Open, FileAccess.Read, FileShare.None, 62500)
                session.SendingFile = True
                BeginFileSend = True
                
                While session.initiatingSendFile
                    Thread.Sleep(1)
                End While

                session.filebytesSent = 0
            Catch ex As Exception
                BeginFileSend = False
                _path = ex.Message
            ...
            If session.SendingFile And Not session.SystemBytesToBeSentAvailable Then
                Try
                    ...
                    If bytesRead > 0 Then
                        ...
                        session.SystemBytesToBeSentAvailable = True
                        session.filebytesSent += bytesRead

                        If session.outgoingFileProgress IsNot Nothing Then
                            ThreadPool.QueueUserWorkItem(Sub()
                                                             session.outgoingFileProgress(session.GetPercentOfFileSent())
                                                         End Sub)
                        End If
                    Else
                        ' Clean up
                        ...
                        If session.outgoingFileProgress IsNot Nothing Then
                            ThreadPool.QueueUserWorkItem(Sub()
                                                             session.outgoingFileProgress(100)
                                                         End Sub)
                        End If
        ...
        Private Function BeginToReceiveAFile(ByVal _path As String, ByVal session As SessionCommunications) As Boolean
            ...
            Try
                ...
                session.fileWriter = New FileStream(_path, FileMode.Create, FileAccess.Write, FileShare.None, 62500)
        ...
        Private Function HandleIncomingFileBytes(ByRef bytes() As Byte, ByVal session As SessionCommunications) As Boolean
            ...
            Try
                ...
                session.fileWriter.Write(bytes, 0, bytes.Length)
            Catch ex As Exception
                Return False
            End Try
        ...
        Private Sub FinishReceivingTheFile(ByVal session As SessionCommunications)
            ...
            'While isRunning AndAlso Now < timeout
            '    SyncLock session.GetFileCompletionLock
            '        If session.writeQueue.Count = 0 Then Exit While
            '    End SyncLock
            '    Thread.Sleep(1)
            'End While
            ...
            ThreadPool.QueueUserWorkItem(Sub()
                                             session.ReceivingFile = False
                                             If session.incomingFileComplete IsNot Nothing Then session.incomingFileComplete()
                                         End Sub)
            
        End Sub

        Private Sub HandleIncomingSystemMessages(ByRef bytes() As Byte, ByVal channel As Byte, ByRef session As SessionCommunications)
            ...
            If channel = 254 Then ' Text commands / messages passed between server and client
                    ...
                    If File.Exists(filePath) Then
                        ...
                        If CheckSessionPermissions(session, "GFR") Then
                            ...
                            If session.SendingFile then
                                ' We are - but maybe we're finishing up the last one. 
                                ' Give the server 2 seconds to mark session.SendingFile False
                                Dim timeout As Date = Now.AddSeconds(2)
                                While Now < timeout And session.SendingFile = False And IsRunning
                                    Thread.Sleep(1)
                                End While
                            End If
                            If Not session.SendingFile Then
                                Dim _theFilesInfo As New FileInfo(filePath)
                                session.outgoingFileSize    = _theFilesInfo.Length
                                session.OutgingFileName     = filePath
                ...
                If tmp = "Sending:" Then
                    ...
                    If session.ReceivingFile Then
                        ' Maybe we're finishing up with the last transfer. Let's wait a few of seconds:
                        Dim timeout As Date = Now.AddSeconds(3)
                        While Now < timeout Andalso session.ReceivingFile AndAlso session.IsRunning
                            Thread.Sleep(1)
                        End While

                        If session.ReceivingFile Then
                            SystemMessage("ERR: A session may only receive one file at a time. This session is already receiving a file.")
                ...
                If message = "<-Done" Then
                    ...
                    SendExternalSystemMessage("<-complete", session)
                    SystemMessage("<-Done")
                End If
                ...
                ' The client has finished processing the file,
                ' and is ready mfor another.
                If message = "->complete" Then
                    Dim thisSession = session
                    ThreadPool.QueueUserWorkItem(Sub()
                                                     thisSession.SendingFile = False
                                                     If thisSession.outgoingFileComplete IsNot Nothing Then thisSession.outgoingFileComplete()
                                                 End Sub)
                End If
            ...
            ElseIf channel = 252 Then ' File transfer from client to server
                Try
                    If session.ReceivingFile Then
                        ...
                        If session.incomingFileProgress IsNot Nothing Then
                            Dim thisSession = session
                            ThreadPool.QueueUserWorkItem(Sub()
                                                             thisSession.incomingFileProgress(thisSession.GetPercentOfFileReceived())
                                                         End Sub)
                        End If
                    End If
                Catch ex As Exception
        ...
        Private Function HandleOutgoingInternalSystemMessage(ByVal Stream As NetworkStream, _
                                                             ByRef session As SessionCommunications) As Boolean
            Dim tmp(1) As Byte
            Dim _size As UShort
            ...
            If session.SystemBytesToBeSentAvailable = True Then
                ...
                End If
            Else
                ' There's no system data to send. Check with the user data:
                If session.UserBytesToBeSentAvailable Or session.sendQueue.Count > 0 Then
                    HandleOutgoingUserData(Stream, session)
                End If
            End If

        End Function

        Private Function HandleOutgoingUserData(ByRef Stream As NetworkStream, ByRef session As SessionCommunications) As Boolean
            Dim tmp(1) As Byte
            Dim _size As UShort
            ...
            If session.UserBytesToBeSentAvailable = True Then
                ...
                Return True
            Else
                ' There was no user data to send. Check to see if there was 
                ' System data that needed to be sent:
                If session.SystemBytesToBeSentAvailable Or session.SendingFile Then
                    HandleOutgoingInternalSystemMessage(Stream, session)
                End If
                Return False
            End If
        End Function
        ...
        Private Sub Run(ByVal _session As Object)
            ...
            Dim remainBytesThisSecond As Integer = 0
            ...
            Try
                ...
                Do
                    ...
                    If bandwidthTimer >= Now And bandwidthUsedThisSecond >= (Mbps / 16) Then
                        While bandwidthTimer > Now
                            Thread.Sleep(1)
                        End While
                    End If
                    If bandwidthTimer <= Now Then
                        bandwidthTimer = Now.AddMilliseconds(65)

                        bandwidthUsedThisSecond = CType(session.bytesRecievedThisSecond - (Mbps / 16), Int32)
                    ...
                    If Server.Client.Available > 0 AndAlso bandwidthUsedThisSecond < (Mbps / 16) Then
                        ...
                        Else
                            ...
                            Do
                                bandwidthUsedThisSecond = session.bytesSentThisSecond + session.bytesRecievedThisSecond
                                remainBytesThisSecond = CType((Mbps / 16) - bandwidthUsedThisSecond, Integer)

                                If remainBytesThisSecond > 0 Then
                                    
                                    If remainBytesThisSecond < (packetSize - bytesread) Then
                                        ' Read bytes in...
                                        thisRead = Stream.Read(theBuffer, bytesread, remainBytesThisSecond)
                                        bytesread += thisRead
                                        'bytesread += Stream.Read(theBuffer, bytesread, remainBytesThisSecond)
                                        session.bytesRecievedThisSecond += thisRead
                                    Else
                                        ' Read bytes in...
                                        thisRead = Stream.Read(theBuffer, bytesread, (packetSize - bytesread))
                                        bytesread += thisRead
                                        session.bytesRecievedThisSecond += thisRead
                                    End If
                                End If

                                '' Do we need to throttle during a packet read?
                                bandwidthUsedThisSecond = session.bytesSentThisSecond + session.bytesRecievedThisSecond
                                If bandwidthTimer >= Now And bandwidthUsedThisSecond >= (Mbps / 16) Then
                                    While bandwidthTimer > Now
                                        Thread.Sleep(1)
                                ...
                                End If
                                If bandwidthTimer <= Now Then
                                    bandwidthTimer = Now.AddMilliseconds(83)
                                    session.bytesRecievedThisSecond = 0
                                    session.bytesSentThisSecond = 0
                                    bandwidthUsedThisSecond = 0
                                End If
                                ...
                                'session.bytesRecievedThisSecond += bytesread
                    ...
                    If Now > idleTimer.AddMilliseconds(25) Then
                        Thread.Sleep(1)
                        If Now > idleTimer.AddSeconds(2) Then
                            Stream.Write(puck, 0, 1)

                            ' Set the idletimer to send it's own puck every 2
                            ' seconds while we're idle so we can make sure we are alerted
                            ' if we loose connection to the client:
                            idleTimer = Now
                        End If

                        If Now > idleTimer.AddSeconds(1) AndAlso session.ReceivingFile Then
                            Stream.Write(puck, 0, 1)

                            ' Set the idletimer to send a puck every second
                            ' while we're idle so we can make sure we are alerted
                            ' if we loose connection to the server:
                            idleTimer = Now
                        End If
                    End If

                    ' Check to see if our last send attempt failed. if so, we're disconnected.
                    If session.isRunning Then session.isRunning = session.theClient.Client.Connected
                    If session.IsRunning = False then Exit Do
                Loop
