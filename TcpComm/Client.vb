...
Public Class Client
        ...
        Private fileWriter As FileStream 'AsyncUnbuffWriter
        ...
        Private connectionRejected As Boolean = False
        Private disConnectComplete As Boolean
        Private initiatingGetFile As Boolean = False
        Private initiatingSendFile As Boolean = False
        ...
        Private Class ReconnectData
            ...
            Public ReconnectOnDisconnection As Boolean
        ...
        Public Function ReceivingAFile() As Boolean
            Return ReceivingFile
        End Function

        Public Function SendingAFile() As Boolean
            Return SendingFile
        End Function
        ...
        Public Function Connect(ByVal IP_Address As String, ByVal prt As UInt16, Optional newMachineID As String = "", _
        ...
        Public Function GetSendQueueSize() As Int32
            ...
            If isRunning then sendQueueSize = sendQueue.Count
            Return sendQueueSize
        End Function

        Private GetFileThreadLock As New Object
        Public Function GetFile(ByVal _path As String, Optional ByVal blockUntilReceiving As Boolean = True, Optional ByRef errorMsg As String = "") As Boolean

            SyncLock (GetFileThreadLock)
                If shuttingDown Then
                    errMsg = "The client is shutting down. Files can not be sent or received."
                    Return False
                End If
                ...
                If blockUntilReceiving Then
                    While (initiatingGetFile AndAlso isRunning AndAlso Now < timeout)

                        If ReceivingFile Then 
                            initiatingGetFile = False
                            Return True
                        End If
        ...
        Private SendFileThreadLock As New Object
        Public Function SendFile(ByVal _path As String, Optional ByRef errorMsg As String = "", Optional ByVal blockUntilSending As Boolean = True) As Boolean

            SyncLock(SendFileThreadLock)
                If shuttingDown then
                ...
                If blockUntilSending Then
                    While (initiatingSendFile AndAlso isRunning AndAlso Now < timeout)
                        Thread.Sleep(1)
                        If SendingFile then 
        ...
        Private GetFileCompletionLock As Object = New Object()
        Private Function BeginToReceiveAFile(ByRef _path As String) As Boolean
            fileBytesRecieved = 0

            Try
                CreateFolders(_path)
                fileWriter          = New  FileStream(_path, FileMode.Create, FileAccess.Write, FileShare.None, 62500)
                ReceivingFile       = True

                While initiatingGetFile
                    Thread.Sleep(1)
                End While

                Return True
            Catch ex As Exception
                _path = "Could not create local copy of file at ''" & _path & "''. The error returned is: " & ex.Message
                errMsg = _path
                Try
                    fileWriter.Close()
        ...
        Private Sub FinishReceivingTheFile()
            ...
            ThreadPool.QueueUserWorkItem(Sub()
                                             Thread.Sleep(5)
                                            ReceivingFile = False
                                            If incomingFileComplete IsNot Nothing Then incomingFileComplete()
                                         End Sub)
        End Sub
        ...
        Private Function BeginFileSend(ByRef _path As String, ByVal fileLength As Long) As Boolean
            filebytesSent = 0

            Try

                fileReader = New FileStream(_path, FileMode.Open, FileAccess.Read, FileShare.None, 62500)
                SendingFile = True
                BeginFileSend = True

                While initiatingSendFile
                    Thread.Sleep(1)
                End While
            ...
            If SendingFile And Not SystemBytesToBeSentAvailable Then
                Try
                    ...
                    If bytesRead > 0 Then
                        ...
                        If outgoingFileProgress IsNot Nothing Then
                            ThreadPool.QueueUserWorkItem(Sub()
                                                             outgoingFileProgress(GetPercentOfFileSent())
                                                         End Sub)
                        End If
                    Else
                        ' Clean up
        ...
        Private Sub HandleIncomingSystemMessages(ByVal bytes() As Byte, ByVal channel As Byte)
            ...
            If channel = 254 Then ' Text commands / messages passed between server and client
                ...
                If tmp = "GFR:" Then ' Get File Request
                    ...
                    If File.Exists(message.Substring(4, message.Length - 4)) Then
                        ...
                        If SendingFile Then
                            ...
                            While Now < timeout And SendingFile And isRunning
                                Thread.Sleep(1)
                            End While
                        End If
                        If Not SendingFile Then
                            ...
                            If BeginFileSend(filePath, _theFilesInfo.Length) Then
                                ...
                                FileBeingSentPath = outgoingFileName
                ...
                If message = "->Done" Then
                    FinishReceivingTheFile()
                    SendExternalSystemMessage("->complete")
                    SystemMessage("->Done")
                End If
                ...
                ' The server has finished writing the file, and has
                ' notified the end user that it is complete.
                If message = "<-complete" Then
                    ThreadPool.QueueUserWorkItem(Sub()
                                                     'Thread.Sleep(10)
                                                     SendingFile = False
                                                     If outgoingFileComplete IsNot Nothing Then outgoingFileComplete()
                                                 End Sub)
                End If
            ...
            ElseIf channel = 253 Then ' File transfer from server to client
                Try
                    If ReceivingFile Then
                        ...
                        If incomingFileProgress IsNot Nothing Then
                            ThreadPool.QueueUserWorkItem(Sub()
                                                             incomingFileProgress(GetPercentOfFileReceived())
                                                         End Sub)
                        End If
                    End If
                Catch ex As Exception
        ...
        Private Function HandleOutgoingInternalSystemMessage() As Boolean
            ...
            Else
                ...
                If UserBytesToBeSentAvailable Or sendQueue.Count > 0 Then
                    Return HandleOutgoingUserData()
                End If
            End If
        ...
        Private Function HandleOutgoingUserData() As Boolean
            ...
            Else
                ...
                If SystemBytesToBeSentAvailable Or SendingFile Then
                    Return HandleOutgoingInternalSystemMessage()
                End If
                ...
                While isRunning
                    ...
                    If weHaveThePuck Then
                        ...
                        Select Case userOrSystemSwitcher
                            Case 1
                                If HandleOutgoingUserData() Then idleTimer = Now
                            Case 2
                                If HandleOutgoingInternalSystemMessage() then idleTimer = Now
                        End Select
                    ...
                    If Now > idleTimer.AddMilliseconds(25) Then
                        ...
                        If Now > idleTimer.AddSeconds(1) AndAlso ReceivingFile Then
                            Stream.Write(puck, 0, 1)
                    ...
                    If isRunning Then isRunning = Client.Client.Connected
                End While
            Catch ex As Exception
